...
par(mfrow=c(2,3))
# scatter for alpha
plot(post_linear$alpha, main="Linear ANCOVA", 
     ylab="alpha", xlab="Iteration", pch=20, col=rgb(0.2,0.5,0.8,0.3),
     ylim = c(0, 6))
plot(post_nl_constant$alpha, main="Non-linear Constant",
     ylab="alpha", xlab="Iteration", pch=20, col=rgb(0.8,0.2,0.5,0.3),
     ylim = c(0, 6))
plot(post_nl_varying$alpha, main="Non-linear Varying",
...
g_alpha_data <- data.frame(
  Linear = c(post_linear$g_alpha[,1], post_linear$g_alpha[,2], post_linear$g_alpha[,3]),
  NonLinear_Constant = c(post_nl_constant$g_alpha[,1], post_nl_constant$g_alpha[,2], post_nl_constant$g_alpha[,3]),
  NonLinear_Varying = c(post_nl_varying$g_alpha[,1], post_nl_varying$g_alpha[,2], post_nl_varying$g_alpha[,3]),
