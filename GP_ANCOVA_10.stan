functions {
  vector gp_pred_rng(array[] vector X2, // testing point
                     ...
                     vector g_rho
                     // , // group GP length scale
                     // array[] vector beta_j, //  group linear 
                     // vector mu_j
  ...
  // calculate total kernel K_x1_x1 and K^-1*y1 ############# 
  {
    ...
    // K_X1_X1  +=  K_group + X1_mat*X1_mat'; // without zero_sum
  ...
  // calcule total kernel K_x1_x2 and predictive distribution variance component #########################
  {
    ...
    // K_X1_X2  += K_group_X1_X2 + X1_mat*X2_mat';
 ...
 // calculate total kernel K_x2_x2 to be used in cov_f2 ############### Initiall I was wrong this part, forgetting combine grand non-linear kernel with group-specific thingy, as well as zero-sum, thus resulting in a large number of Non-positive definite matrix warnings, effectiveless sampling.
  ...
  // END calculating total kernel K_x2_x2 #########################
      ...
      K_div_Y1 = mdivide_left_tri_low(L_K, Y1); // this step is to compute components of the posterior predictive distribution P(f*|Y), same thing was done as well in the stan user guide
      ...
      diag_delta = diag_matrix(rep_vector(delta, N2)); // add the jitter (seemingly I add the jitter twice for the training elements in the COV??)
...
transformed data {
  ...
  matrix[J, J] zero_sum_beta_g;
  real off_diag = -1.0 / (J - 1);
  for (i in 1:J)
    for (j in 1:J)
      zero_sum_beta_g[i, j] = (i == j) ? 1.0 : off_diag;
}

parameters {
  real<lower=0> lambda_global; //global regularized horseshoe prior
  real<lower=0> c_global2; //global regularized horseshoe prior
  vector<lower=0>[J] lambda_group;  //group regularized horseshoe prior
  vector<lower=0>[J] c_group2; //global regularized horseshoe prior
  
  real<lower=0> rho;            // GP length-scale (non-group specific)
  vector[J] mu_j;                 // group effect (group-specific, mean)
  matrix[J, K] beta_g_raw;  // unconstrained group linear component
...
transformed parameters{
  real<lower=0> alpha = sqrt((c_global2*(lambda_global^2)) / (c_global2 + lambda_global^2));;
  vector<lower=0>[J] g_alpha;
      for (j in 1:J) {
      g_alpha[j] =  sqrt((c_group2[j]*(lambda_group[j]^2)) / (c_group2[j] + lambda_group[j]^2));
    }
  matrix[J, K] beta_j = beta_g_raw * zero_sum_beta_g;
}
...
model {
  ...
  // mean function part: only mu_j, beta integrated out (in GP_ANCOVA_10, add group-specific linear part beta_j)
  matrix[N1, K] beta_n;
  for (n in 1:N1){
    beta_n[n] = beta_j[group1[n]];
  }
  vector[N1] mu = mu_a + mu_j[group1] + rows_dot_product(beta_n, X1_mat);

  // priors
  rho ~ inv_gamma(5, 5);
  g_rho ~ inv_gamma(5, 5);
  to_vector(beta_g_raw) ~ std_normal();
  mu_j ~ std_normal();
  sigma ~ std_normal();
  // beta ~ std_normal();
  Y1 ~ multi_normal_cholesky(mu, L_K);
  
  // regularized shrinkage horseshoe priors
  // GP(0, alpha2*Kernel(X, X'))
  // alpha2 = c^2 * lambda^2 / (c^2 + lambda^2)
  // lambda ~ Half-Cauchy(0, 1)
  c_global2 ~ inv_gamma(2, 8);
  c_group2 ~ inv_gamma(2, 8);
  lambda_global ~ cauchy(0, 1);
  lambda_group ~ cauchy(0, 1);
}
generated quantities {
  vector[N2] f2;
  vector[N2] y2;

  f2 = gp_pred_rng(X2, Y1, J, K, X1, group1, group2, alpha, rho, sigma, delta, g_alpha, g_rho);
  for (n2 in 1:N2){
