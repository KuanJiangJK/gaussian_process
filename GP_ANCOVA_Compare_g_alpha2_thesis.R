...
generate_group_shift_dataset <- function(t, N = 100, J = 3, sigma = 1) {
  X <- runif(N, -2, 2)
  group <- sample(1:J, N, prob = rep(1/J,J),replace = TRUE)
  # Fixed grand nonlinear effect
  grand_nonlinear <- function(x) 5 * (sin(2 * x) + x^2)
  # Fixed grand linear effect
  grand_linear <- function(x) 5*x
  # Varying group-specific nonlinear shift
  shift_strength <- c(0, 0, 0) # initial shift strength (coef before the linear part)
  constant_shift <- c(-10, 0, 10) # constant shift 
  if (t >= 2) shift_strength[1] <- (t - 1) * 8  # group 1 gets increasing nonlinear shift relative to the global effect
  group_shift <- function(x, g) shift_strength[g] * (cos(x * pi / 2) + 0.5*x^2)+ constant_shift[g]
  # y <- map_dbl(seq_along(X), ~ group_shift(X[.x], group[.x]) + grand_linear(X[.x]) + 0* grand_nonlinear(X[.x]) + rnorm(1, 0, sigma))
  y <- map_dbl(seq_along(X), ~ group_shift(X[.x], group[.x]) + grand_nonlinear(X[.x])  + rnorm(1, 0, sigma))
      # map_dbl.: Apply a function to each element of a vector or list
      # NOTE: .x is the current sequence number of the X. It performs indexing of .x'th value inside the vector X
  tibble(x = X, y = y, group = group, scenario = paste0("t=", t))
}

ancova_scenarios <- map_dfr(1:4, generate_group_shift_dataset) ## plot the 4 scenario
...
for (t in 1:4) {
  set.seed(100 + t)
  df <- generate_group_shift_dataset(t)
  stan_data_list[[t]] <- prepare_stan_data(df)
  assign(paste0("stan_data_", t), stan_data_list[[t]])
}

# ----------------------- 3. Run Stan models for each t ----------------------- #
model_files <- c("GP_ANCOVA_g_uniform.stan", "GP_ANCOVA_7.stan", "GP_ANCOVA_g_normal.stan")
