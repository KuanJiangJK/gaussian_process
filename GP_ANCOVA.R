...
######### Session for GP_ANOCVA_3.stan (with horseshoe prior) ####################
fit3 <- stan(
...
posterior_df3 <- as.data.frame(fit3)

# Parameters to trace
par(mfrow=c(2,3))
plot(fit3, pars = "alpha", plotfun = "stan_trace")
plot(fit3, pars = "rho", plotfun = "stan_trace")
plot(fit3, pars = "sigma", plotfun = "stan_trace")
plot(fit3, pars = "beta", plotfun = "stan_trace")
plot(fit3, pars = "mu", plotfun = "stan_trace")
plot(fit3, pars = "g_alpha", plotfun = "stan_trace")
plot(fit3, pars = "g_rho", plotfun = "stan_trace")
par(mfrow=c(1,1))
plot(density(post3$alpha))
abline(v = mean(post3$alpha), col = "red")
abline(v = median(post3$alpha), col = "blue")
...
# test inputs
N_test <- nrow(X2)
f_samples <- post3$F  # dimension: iterations x N_obs + N_test
# Extract only test part
f_test_samples <- f_samples[, (nrow(X1) + 1):(nrow(X1) + N_test)]
# sampling 10 per group
draws <- sample(1e3, 10)
